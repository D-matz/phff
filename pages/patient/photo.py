...
from test.resources import Patient, Attachment, OperationOutcome
import html
import json

@app.get("/patient/{patient_id}/photo", name="patient_photo_get")
async def patient_photo_get(patient_id: str):
    ...
    return base_patient_nav(all_resources, photo_form(patient))
...
async def patient_photo_post(patient_id: str, patient: Patient):
    ...
    try:
        ...
        e_oo: OperationOutcome = client.resource('OperationOutcome', **json.loads(str(e)))
        if e_oo.issue and e_oo.issue[0] and e_oo.issue[0].diagnostics:
            errors = e_oo.issue[0].diagnostics
        elif e_oo.text and e_oo.text.div:
            errors = e_oo.text.div
    ...
    all_resources = await get_all_resources(patient_id)
    patient: Patient = all_resources.patient
    return base_patient_nav(all_resources, photo_form(patient, errors=errors))
...
def photo_form(patient: Patient, errors: str=None) -> str:
    photo = None
    if patient.photo:
...
.grow-hover:hover {{ transform: scale(1.05); }}
    ...
    lines.append(f"""
        ...
        <div class="color-color3 color-color3-hover" style="border: dashed 2px; width: fit-content; padding: 4px;">
